...
const InventoryTurnover = () => {
  ...
  function calculate() {
    ...
    const avg = parseFloat(avgInventory);
    if (isNaN(cogs) || isNaN(avg) || avg === 0) {
      setResult("Invalid input");
      setLatex("");
      return;
    }
    const turnover = cogs / avg;
    setResult(turnover.toFixed(2));
    setLatex(`\\text{Inventory Turnover} = \\frac{\\text{Cost of Goods Sold}}{\\text{Average Inventory}} = \\frac{${cogs}}{${avg}} = ${turnover.toFixed(2)}`);
    setLogs([`COGS: $${cogs}`, `Average Inventory: $${avg}`, `Turnover: ${turnover.toFixed(2)}`]);
  }
  ...
  const handleBatch = () => {
    ...
    rows.forEach((row, idx) => {
      const [cogs, avg] = row.split(",");
      const c = parseFloat(cogs as string), a = parseFloat(avg as string);
      if (!isNaN(c) && !isNaN(a) && a !== 0) {
        const t = c / a;
        results.push(`Row ${idx + 1}: Inventory Turnover = ${t.toFixed(2)}`);
      }
    });
    setLogs(results);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 pt-8 pb-12">
        <Card className="max-w-xl mx-auto animate-fade-in">
          <CardHeader>
            <CardTitle className="text-2xl">Inventory Turnover Calculator</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium">
                  Cost of Goods Sold ($){renderTooltip("Total cost of goods sold for the period")}
                </label>
                <Input value={costOfGoodsSold} onChange={e => setCostOfGoodsSold(e.target.value)} type="number" />
              </div>
              <div>
                <label className="block text-sm font-medium">
                  Average Inventory ($){renderTooltip("Average value of inventory during the period")}
                </label>
                <Input value={avgInventory} onChange={e => setAvgInventory(e.target.value)} type="number" />
              </div>
              ...
              {result && (
                <div className="mt-4 p-3 rounded bg-gray-50 text-sm">
                  <strong>Inventory Turnover:</strong> {result}
                  {latex && (
            ...
            {showAdvanced && (
              <div className="mt-4 border-t pt-4">
                <div>
                  <label className="block text-sm font-medium">
                    Batch Calculation Input {renderTooltip("Paste: Cost of Goods Sold,Average Inventory (CSV)")}
                  </label>
                  <textarea
                    className="w-full h-16 rounded border px-2 py-1 text-xs"
                    value={batchInput}
                    onChange={e => setBatchInput(e.target.value)}
                    placeholder="35000,5600"
                  />
