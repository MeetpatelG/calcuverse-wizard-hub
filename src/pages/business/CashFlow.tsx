...
const CashFlow = () => {
  ...
  function calculate() {
    const infl = parseFloat(inflow), outfl = parseFloat(outflow);
    if (isNaN(infl) || isNaN(outfl)) {
      setResult("Invalid input");
      setLatex("");
      return;
    }
    const cashFlow = infl - outfl;
    setResult(`$${cashFlow.toLocaleString()}`);
    setLatex(`\\text{Cash Flow} = \\text{Inflow} - \\text{Outflow} = ${infl} - ${outfl} = ${cashFlow}`);
    setLogs([`Inflow: $${infl}`, `Outflow: $${outfl}`, `Cash Flow: $${cashFlow}`]);
  }
  ...
  const handleBatch = () => {
    ...
    rows.forEach((row, idx) => {
      const [inflow, outflow] = row.split(",");
      const infl = parseFloat(inflow as string), outfl = parseFloat(outflow as string);
      if (!isNaN(infl) && !isNaN(outfl)) {
        const cf = infl - outfl;
        results.push(`Row ${idx + 1}: Cash Flow = $${cf}`);
      }
    });
    setLogs(results);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 pt-8 pb-12">
        <Card className="max-w-xl mx-auto animate-fade-in">
          <CardHeader>
            <CardTitle className="text-2xl">Cash Flow Calculator</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium">
                  Cash Inflow ($){renderTooltip("Total money received")}
                </label>
                <Input value={inflow} onChange={e => setInflow(e.target.value)} type="number" />
              </div>
              <div>
                <label className="block text-sm font-medium">
                  Cash Outflow ($){renderTooltip("Total money spent")}
                </label>
                <Input value={outflow} onChange={e => setOutflow(e.target.value)} type="number" />
              </div>
              ...
              {result && (
                <div className="mt-4 p-3 rounded bg-gray-50 text-sm">
                  <strong>Cash Flow:</strong> {result}
                  {latex && (
            ...
            {showAdvanced && (
              <div className="mt-4 border-t pt-4">
                <div>
                  <label className="block text-sm font-medium">
                    Batch Calculation Input {renderTooltip("Paste rows: Inflow,Outflow (CSV)")}
                  </label>
                  <textarea
                    className="w-full h-16 rounded border px-2 py-1 text-xs"
                    value={batchInput}
                    onChange={e => setBatchInput(e.target.value)}
                    placeholder="12000,8000"
                  />
