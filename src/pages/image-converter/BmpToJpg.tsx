...
const BmpToJpg = () => {
  ...
  // Accept only BMP images
  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    setError(null);
    const f = e.target.files?.[0];
    if (!f) return;
    if (!f.type.match(/^image\/bmp$/)) {
      setError("Only .bmp files are supported.");
      return;
    }
    handleInput(f);
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="max-w-xl mx-auto bg-card shadow-lg rounded-lg p-8 text-center">
          ...
          <h1 className="text-2xl font-bold mb-2">BMP to JPG Converter</h1>
          <p className="mb-6 text-muted-foreground">
            Convert BMP files to JPG format instantly, right in your browser.
          </p>
