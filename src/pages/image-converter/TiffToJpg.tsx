...
const TiffToJpg = () => {
  ...
  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (
      !(
        f.type.match(/^image\/tiff$/) ||
        (f.name && f.name.match(/\.(tif|tiff)$/i))
      )
    ) {
      setError("Only .tif or .tiff files are supported.");
      return;
    }
    handleInput(f);
  }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-12">
        <div className="max-w-xl mx-auto bg-card shadow-lg rounded-lg p-8 text-center">
          ...
          <h1 className="text-2xl font-bold mb-2">TIFF to JPG Converter</h1>
          <p className="mb-6 text-muted-foreground">
            Convert TIFF files to JPG format instantly, right in your browser.
          </p>
